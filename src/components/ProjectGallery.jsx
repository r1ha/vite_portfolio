    ...
    <section className="mt-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="hidden sm:flex gap-2">
          <button aria-label="scroll left" onClick={() => scrollBy(-300)} className="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M15 18L9 12L15 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button aria-label="scroll right" onClick={() => scrollBy(300)} className="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        {projects.map(p => (
          <article key={p.id} className="snap-center flex-shrink-0 w-80 sm:w-64 bg-white border rounded-xl shadow-sm p-4">
            <div className="w-full h-36 bg-gray-100 rounded-md overflow-hidden mb-3">
              {p.image ? <img src={p.image} alt={p.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-neutral-400">No image</div>}
            </div>
